...
├── uploads/                   resumable-upload parts (survive a restart; NOT swept)
└── tmp/                       upload staging (same fs → atomic rename on commit)
```

`env.ts` resolves every path from the in-container `DATA_DIR` (default `/data`):
`dbPath = data/photodrop.db`, `albumsDir = albums/`, `uploadsDir = uploads/`,
`tmpDir = tmp/`. No data path is hardcoded — relocating the volume never touches code.
