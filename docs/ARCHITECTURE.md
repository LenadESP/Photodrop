...
├── albums/<uid>/display/      WebP display derivatives (~2560px), for the lightbox
...
5. **Worker** (`plugins/thumbnailer.ts`) — one photo at a time (`sharp.concurrency(1)`;
   ...
   ~2560px display derivative, then exiftool strips the original if `album.exif_strip`,
...
Endpoints: `/thumb/:id` (grid), `/display/:id` (lightbox — the ~2560px derivative, or
...
viewer paints the screen from a ~2560px image instead of a full 24 MP original; the
